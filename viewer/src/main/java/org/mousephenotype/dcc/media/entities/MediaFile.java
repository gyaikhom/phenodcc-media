...
@NamedQueries({
    ...
    @NamedQuery(name = "MediaFile.find", query = "SELECT new org.mousephenotype.dcc.media.entities.MediaFileDetail(mf.id, m.measurementId, pao.animalId, pao.animalName, m.genotypeId, pao.zygosity, pao.sex, pao.startDate, pao.equipmentmanufacturer, pao.equipmentmodel, mf.checksum, mf.isImage, e.extension, mf.width, mf.height, mf.phaseId.id, mf.statusId.id, pao.metadataGroup, l.pipelineId) FROM ProcedureAnimalOverview pao, Pipeline l, Procedure p, Parameter q, MeasurementsPerformed m, MediaFile mf, FileExtension e WHERE pao.centreId = :cid AND pao.pipeline = l.pipelineKey AND l.pipelineId = :lid AND (pao.genotypeId = :gid or pao.genotypeId = 0) AND pao.strainId = :sid AND pao.procedureId = p.procedureKey AND p.procedureId = :pid AND pao.procedureOccurrenceId = m.procedureOccurrenceId AND m.parameterId = q.parameterKey AND q.parameterId = :qid AND mf.mid = m.measurementId AND mf.extensionId.id = e.id ORDER BY pao.animalName"),
    @NamedQuery(name = "MediaFile.findIgnorePipeline", query = "SELECT new org.mousephenotype.dcc.media.entities.MediaFileDetail(mf.id, m.measurementId, pao.animalId, pao.animalName, m.genotypeId, pao.zygosity, pao.sex, pao.startDate, pao.equipmentmanufacturer, pao.equipmentmodel, mf.checksum, mf.isImage, e.extension, mf.width, mf.height, mf.phaseId.id, mf.statusId.id, pao.metadataGroup, l.pipelineId) FROM ProcedureAnimalOverview pao, Pipeline l, Procedure p, Parameter q, MeasurementsPerformed m, MediaFile mf, FileExtension e WHERE pao.centreId = :cid AND pao.pipeline = l.pipelineKey AND (pao.genotypeId = :gid or pao.genotypeId = 0) AND pao.strainId = :sid AND pao.procedureId = p.procedureKey AND p.procedureId = :pid AND pao.procedureOccurrenceId = m.procedureOccurrenceId AND m.parameterId = q.parameterKey AND q.parameterId = :qid AND mf.mid = m.measurementId AND mf.extensionId.id = e.id ORDER BY pao.animalName")
})
