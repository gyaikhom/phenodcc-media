...
@NamedQueries({
    ...
    @NamedQuery(name = "MediaFile.findMutantMediaFilesProcedurePipeline", query = "SELECT new org.mousephenotype.dcc.media.entities.MediaFileDetail(mf.id, mp.animalId, mp.animalName, mp.genotypeId, mp.zygosity, mp.sex, mp.startDate, mf.checksum, mf.isImage, e.extension, mf.width, mf.height, mf.phaseId.id, mf.statusId.id, mp.metadataGroup, l.pipelineId, p.procedureId, q.parameterId, mp.measurementId, a.assocQeid, a.assocName, s.link) FROM MeasurementsPerformed mp LEFT JOIN MediaFile mf ON (mf.mid = mp.measurementId) LEFT JOIN FileExtension e ON (e = mf.extensionId) LEFT JOIN ProcedureAnimalOverview pao ON (pao.procedureOccurrenceId = mp.procedureOccurrenceId) LEFT JOIN Procedure p ON (p.procedureKey = pao.procedureId) LEFT JOIN Pipeline l ON (l.pipelineKey = pao.pipeline) LEFT JOIN Parameter q ON (q.parameterKey = mp.parameterId) LEFT JOIN Association a ON (a.mid = mp.measurementId) LEFT JOIN Seriesmediaparametervalue s ON (s.hjid = mp.measurementId) WHERE mp.centreId = :centreId AND mp.genotypeId = :genotypeId AND mp.strainId = :strainId AND mp.parameterId = :parameterKey AND p.procedureId = :procedureId AND l.pipelineId = :pipelineId ORDER BY mp.animalName"),
    @NamedQuery(name = "MediaFile.findBaselineMediaFilesProcedurePipeline", query = "SELECT new org.mousephenotype.dcc.media.entities.MediaFileDetail(mf.id, mp.animalId, mp.animalName, mp.genotypeId, mp.zygosity, mp.sex, mp.startDate, mf.checksum, mf.isImage, e.extension, mf.width, mf.height, mf.phaseId.id, mf.statusId.id, mp.metadataGroup, l.pipelineId, p.procedureId, q.parameterId, mp.measurementId, a.assocQeid, a.assocName, s.link) FROM MeasurementsPerformed mp LEFT JOIN MediaFile mf ON (mf.mid = mp.measurementId) LEFT JOIN FileExtension e ON (e = mf.extensionId) LEFT JOIN ProcedureAnimalOverview pao ON (pao.procedureOccurrenceId = mp.procedureOccurrenceId) LEFT JOIN Procedure p ON (p.procedureKey = pao.procedureId) LEFT JOIN Pipeline l ON (l.pipelineKey = pao.pipeline) LEFT JOIN Parameter q ON (q.parameterKey = mp.parameterId) LEFT JOIN Association a ON (a.mid = mp.measurementId) LEFT JOIN Seriesmediaparametervalue s ON (s.hjid = mp.measurementId) WHERE mp.centreId = :centreId AND mp.genotypeId = 0 AND mp.strainId = :strainId AND mp.parameterId = :parameterKey AND mp.metadataGroup = :metadataGroup AND p.procedureId = :procedureId AND l.pipelineId = :pipelineId ORDER BY mp.animalName"),
    @NamedQuery(name = "MediaFile.findMutantMediaFiles", query = "SELECT new org.mousephenotype.dcc.media.entities.MediaFileDetail(mf.id, mp.animalId, mp.animalName, mp.genotypeId, mp.zygosity, mp.sex, mp.startDate, mf.checksum, mf.isImage, e.extension, mf.width, mf.height, mf.phaseId.id, mf.statusId.id, mp.metadataGroup, l.pipelineId, p.procedureId, q.parameterId, mp.measurementId, a.assocQeid, a.assocName, s.link) FROM MeasurementsPerformed mp LEFT JOIN MediaFile mf ON (mf.mid = mp.measurementId) LEFT JOIN FileExtension e ON (e = mf.extensionId) LEFT JOIN ProcedureAnimalOverview pao ON (pao.procedureOccurrenceId = mp.procedureOccurrenceId) LEFT JOIN Procedure p ON (p.procedureKey = pao.procedureId) LEFT JOIN Pipeline l ON (l.pipelineKey = pao.pipeline) LEFT JOIN Parameter q ON (q.parameterKey = mp.parameterId) LEFT JOIN Association a ON (a.mid = mp.measurementId) LEFT JOIN Seriesmediaparametervalue s ON (s.hjid = mp.measurementId) WHERE mp.centreId = :centreId AND mp.genotypeId = :genotypeId AND mp.strainId = :strainId AND mp.parameterId = :parameterKey ORDER BY mp.animalName"),
    @NamedQuery(name = "MediaFile.findBaselineMediaFiles", query = "SELECT new org.mousephenotype.dcc.media.entities.MediaFileDetail(mf.id, mp.animalId, mp.animalName, mp.genotypeId, mp.zygosity, mp.sex, mp.startDate, mf.checksum, mf.isImage, e.extension, mf.width, mf.height, mf.phaseId.id, mf.statusId.id, mp.metadataGroup, l.pipelineId, p.procedureId, q.parameterId, mp.measurementId, a.assocQeid, a.assocName, s.link) FROM MeasurementsPerformed mp LEFT JOIN MediaFile mf ON (mf.mid = mp.measurementId) LEFT JOIN FileExtension e ON (e = mf.extensionId) LEFT JOIN ProcedureAnimalOverview pao ON (pao.procedureOccurrenceId = mp.procedureOccurrenceId) LEFT JOIN Procedure p ON (p.procedureKey = pao.procedureId) LEFT JOIN Pipeline l ON (l.pipelineKey = pao.pipeline) LEFT JOIN Parameter q ON (q.parameterKey = mp.parameterId) LEFT JOIN Association a ON (a.mid = mp.measurementId) LEFT JOIN Seriesmediaparametervalue s ON (s.hjid = mp.measurementId) WHERE mp.centreId = :centreId AND mp.genotypeId = 0 AND mp.strainId = :strainId AND mp.parameterId = :parameterKey AND mp.metadataGroup = :metadataGroup ORDER BY mp.animalName")
})
