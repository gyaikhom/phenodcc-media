...
then
    echo "ERROR: Supplied image file does not exists...";
...
# Check if the file to process exists.
if [ -e "${file_to_process}" ]
then
    # Check if we have already generated the tiles
    if [ -d "${checksum_dir}" ]
    then
        echo "        Tile directory ${checksum_dir} already exists... will skip tiling";
    else
    ...
    echo "        File ${file_to_process} does not exists... will abort tiling";
fi;
